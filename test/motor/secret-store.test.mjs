...
import {
  ...
  PROFILE_ACCOUNT,
  resolveStateDir,
} from '../../lib/motor/secret-store.mjs';
...
test('linux store round-trips the profile through an owner-only file', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'jaa-linux-'));
  const store = createSecretStore({ platform: 'linux', stateDir: () => dir });
  assert.throws(() => store.readProfile(), /missing or unreadable/);
  store.writeProfile('{"name":"x"}');
  assert.equal(store.readProfile(), '{"name":"x"}');
  assert.equal((await stat(join(dir, 'profile.json'))).mode & 0o777, 0o600);
});
