...
import { buildReview, commandCategory, durationBucket, migrateProfile, profileStatus, scoreJob, telemetryJobAssessed, validateLedgerEntry, validateProfile, validateSubmissionTelemetry } from '../../lib/motor/job-application.mjs';
...
test('returns the canonical resume path for direct browser uploads', async (t) => {
  ...
  const script = new URL('../../lib/motor/job-application.mjs', import.meta.url).pathname;
...
test('migrates a legacy profile without discarding identity or salary preference', () => {
  ...
  assert.equal(migrated.submissionMode, 'review-each');
...
test('deduplicates ledger entries by normalized URL', async (t) => {
  ...
  const script = new URL('../../lib/motor/job-application.mjs', import.meta.url).pathname;
  const entry = {
    id: 'example-role-1', company: 'Example', role: 'Senior Product Engineer', url: 'https://jobs.example.com/123?utm_source=x', source: 'company', score: 80, status: 'submitted', submittedAt: '2026-01-15T10:00:00Z', approval: 'APPROVE SUBMIT', answers: {},
...
test('warns on same-company role matches and serializes concurrent duplicate submissions', async (t) => {
  ...
  const base = {
    ...
    status: 'submitted', submittedAt: '2026-01-15T10:00:00Z', approval: 'APPROVE SUBMIT', answers: {}, employerJobId: 'example:123',
  };
...
test('records structured outcomes idempotently without duplicate rows', async (t) => {
  ...
  await writeFile(join(directory, 'applications.ndjson'), `${JSON.stringify({
    ...
    status: 'submitted', submittedAt: '2026-01-15T10:00:00Z', approval: 'APPROVE SUBMIT', answers: {},
  })}\n`);
  const env = { ...process.env, JOB_APPLICATION_AGENT_STATE_DIR: directory };
...
test('builds canonical mature-cohort reviews and honors explicit acknowledgement', () => {
  const old = Array.from({ length: 20 }, (_, index) => ({
    ...
    status: 'submitted', submittedAt: '2025-12-01T10:00:00Z', source: 'company', score: 80, approval: 'APPROVE SUBMIT', answers: {},
  }));
  const recent = Array.from({ length: 2 }, (_, index) => ({
    ...
    status: 'submitted', submittedAt: '2026-01-19T10:00:00Z', source: 'company', score: 85, approval: 'APPROVE SUBMIT', answers: {},
  }));
...
test('preserves interview quality after a later final outcome', () => {
  const applications = [{
    ...
    status: 'submitted', submittedAt: '2025-12-01T10:00:00Z', source: 'company', score: 90, approval: 'APPROVE SUBMIT', answers: {},
  }];
  const outcomes = [
...
test('acknowledges a generated review only through the explicit CLI command', async (t) => {
  ...
  const entries = Array.from({ length: 10 }, (_, index) => ({
    ...
    source: 'company', score: 80, status: 'submitted', submittedAt: '2026-01-01T10:00:00Z', approval: 'APPROVE SUBMIT', answers: {},
  }));
