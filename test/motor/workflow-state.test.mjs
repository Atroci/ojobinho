...
test('standing autonomy grants are refused; status stays disabled', async (t) => {
  const { env } = await fixture(t, 'autonomy');
  const refused = cliFailure(env, ['autonomy', 'grant', '--stdin'], { mode: 'routine-auto' });
  assert.notEqual(refused.status, 0);
  assert.match(refused.stderr, /Standing autonomy is disabled/);
  assert.equal(cli(env, ['autonomy', 'status']).enabled, false);
  assert.equal(cli(env, ['autonomy', 'preview']).maySubmitRoutineApplications, false);
});
