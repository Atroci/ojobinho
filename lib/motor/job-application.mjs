...
const SOURCES = new Set(['linkedin', 'indeed', 'gupy', 'catho', 'infojobs', 'trampos', 'vagas-com-br', 'jobgether', 'greenhouse', 'lever', 'ashby', 'workable', 'comeet', 'workday', 'rippling', 'smartrecruiters', 'google-form', 'company', 'email', 'other']);
const DISCOVERY_SOURCES = new Set(['direct-company', 'discovery-worker', 'indeed', 'gupy', 'catho', 'infojobs', 'trampos', 'vagas-com-br', 'linkedin', 'x', 'yc', 'hacker-news', 'job-board', 'email', 'user-supplied', 'web-search', 'other']);
...
const ROLE_FAMILIES = new Set(['paid-media', 'performance-marketing', 'growth-marketing', 'marketing', 'design', 'sales', 'customer-success', 'product-management', 'project-management', 'operations', 'finance', 'people', 'frontend', 'backend', 'full-stack', 'product-engineering', 'ai-ml', 'platform', 'infrastructure', 'mobile', 'engineering-management', 'architecture', 'security', 'data', 'other']);
...
const APPROVALS = new Set(['APPROVE SUBMIT']); // ponytail: every ledger row carries Hugo's explicit per-send approval
const MODES = new Set(['review-each']); // ponytail: Hugo is the final sender (docs/submission-orchestration.md); routine-auto removed on purpose
...
export function validateProfile(input) {
  ...
  if (!MODES.has(profile.submissionMode)) throw new Error('profile.submissionMode must be review-each (Hugo is the final sender; routine-auto is disabled in this deployment).');
...
async function autonomyGrant(input) {
  ...
  throw new Error('Standing autonomy is disabled in this deployment: Hugo approves every send (see docs/submission-orchestration.md).');
}
...
async function executeCommand([area, action, value], telemetry, session) {
  ...
  else throw new Error('Usage: profile set|migrate --stdin; profile check|field <name>; resume import <url-or-pdf>|path; score --stdin; ledger check|add|outcome|review-ack --stdin; ledger review; autonomy grant --stdin|status|preview|revoke; round start|complete --stdin|status [round-id]; attention add|resolve --stdin|list; friction record --stdin|list');
