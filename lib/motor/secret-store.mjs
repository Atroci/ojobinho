...
// ponytail: Linux = owner-only JSON file in the state dir (no libsecret on a headless VPS);
// ledgers already live plaintext there, so the profile gets the same 0600 treatment.
function createLinuxFileStore({ stateDir }) {
  const profilePath = () => join(typeof stateDir === 'function' ? stateDir() : stateDir, 'profile.json');
  return {
    readProfile() {
      try { return readFileSync(profilePath(), 'utf8'); } catch {
        throw new Error('The stored profile is missing or unreadable. Run profile set again.');
      }
    },
    writeProfile(raw) {
...
export function createSecretStore({
  ...
  return createLinuxFileStore({ stateDir });
}
